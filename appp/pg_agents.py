        ...
        with st.container():
            ...
            with tab_objects[0]:
                ...
                for agent in ss.agents:
                    ...
                    if agent.edit:
                        editing = True
                ss.last_agent_key += 1
                st.button('Create agent', on_click=self.create_agent, disabled=editing, key=f"create_agent_all_{ss.last_agent_key}")
            ...
            with tab_objects[1]:
                ...
                for agent in unassigned_agents:
                    unique_key = f"{agent.id}_unassigned_{int(time.time())}"
                    agent.draw(key=unique_key)
                    if agent.edit:
                        editing = True
                ss.last_agent_key += 1
                st.button('Create agent', on_click=self.create_agent, disabled=editing, key=f"create_agent_unassigned_{ss.last_agent_key}")
            ...
            for i, crew in enumerate(ss.crews, 2):
                with tab_objects[i]:
                    ...
                    for agent in assigned_agents:
                        unique_key = f"{agent.id}_{crew.name}_{int(time.time())}"
                        agent.draw(key=unique_key)
                        if agent.edit:
                            editing = True
                    ss.last_agent_key += 1
                    st.button('Create agent', on_click=self.create_agent, disabled=editing, kwargs={'crew': crew}, key=f"create_agent_{crew.name}_{ss.last_agent_key}")

            if len(ss.agents) == 0:
                st.write("No agents defined yet.")
                ss.last_agent_key += 1
                st.button('Create agent', on_click=self.create_agent, disabled=editing, key=f"create_agent_empty_{ss.last_agent_key}")
